...
```{r project.setup, echo=FALSE, include=FALSE}
...
```{r echo=FALSE, fig.align='left', fig.cap="Monthly Ozone 2018", fig.height=3, warning=FALSE}

epa_data%>%
  filter(Year==2018)%>%
  mutate(month.txt=factor(Month))%>%
ggplot(aes(x=month.txt, y= Ozone))+geom_boxplot()


```


```{r echo=FALSE, fig.align='left', fig.cap="Monthly Ozone 2019" , fig.height=3, warning=FALSE}
epa_data%>%
  filter(Year==2019)%>%
  mutate(month.txt=factor(Month))%>%
ggplot(aes(x=month.txt, y= Ozone))+geom_boxplot()
